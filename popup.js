...
function displaySimilarProducts(similarProducts) {
  let similarProductsHTML =
    '<h3 style="font-family: \'Helvetica\', Arial, sans-serif;">Similar Products:</h3><ul style="list-style: none; padding: 0;">';
...
function handleShowSimilarProducts(productName) {
  ...
  chrome.runtime.sendMessage(
    { action: "getSimilarProducts", productName },
    (response) => {
      if (response.similarProducts) {
        productInfoDiv.innerHTML = "";

        // Handle the display of similar products here
        productDiv.innerHTML += displaySimilarProducts(
  ...
  chrome.tabs.sendMessage(
    ...
    (response) => {
      if (response) {
        ...
        `;
        const showSimilarProductsButton = document.getElementById(
        ...
        productInfoDiv.innerHTML = "";
        productDiv.innerHTML =
          "<p>Please refersh the page to load the plugin properly. Product information not found.</p>";
      }
    }
  );
});
